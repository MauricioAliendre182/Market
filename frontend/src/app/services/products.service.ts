...
export class ProductsService {
  ...
  getProductByCategory(categoryId: string, limit?: number, offset?: number){
    ...
    return this.httpClient.get<Product[]>(`${this.url}/category/${categoryId}`, { params })
  }
  ...
  getAllProducts(limit?: number, offset?: number) {
    ...
    return this.httpClient.get<Product[]>(this.url,  {params: params, context:checkTime()})
    .pipe(
      retry(2),
  ...
  getProdcutByPage(limit: number, offset: number) {
    return this.httpClient.get<Product[]>(this.url,
      {params:{limit, offset}})
  }

  getProduct(id: number|string) {
    return this.httpClient.get<Product>(`${this.url}/${id}`, {context: checkTime()})
    .pipe(
      catchError((error: HttpErrorResponse)=>{
        if (error.status === 500) {
